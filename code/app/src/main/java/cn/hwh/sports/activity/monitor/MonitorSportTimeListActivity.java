 ...
 * 运动时间列表详情页面
 */
public class MonitorSportTimeListActivity extends BaseActivity {
    ...
    /* view */
    @BindView(R.id.lv_sport_time)
    PullablePinnedSectionListView mSportTimeLv;
    ...
    protected int getLayoutId() {
        return R.layout.activity_monitor_sport_time_detail_list;
    }
    ...
    Handler mPostHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case MSG_POST_ERROR:
                    T.showShort(MonitorSportTimeListActivity.this, msg.obj.toString());
                    if (!mFirstIn) {
    ...
    protected void initData() {
        mUserId = LocalApplication.getInstance().getLoginUser(MonitorSportTimeListActivity.this).userId;
        mCurrLoadingDay = TimeU.nowDay();
    }

    @Override
    protected void initViews(Bundle savedInstanceState) {
        mAdapter = new MonitorListAdapter(MonitorSportTimeListActivity.this, mMonitorData, MonitorListAdapter.VALUE_TYPE_INT);
        mSportTimeLv.setAdapter(mAdapter);
...
//                doRefresh();
    ...
    protected void onResume() {
        ...
        if (mHasShowChart && !mIsFirstIn) {
            mListHeadME.targetValue = LocalApplication.getInstance().getLoginUser(MonitorSportTimeListActivity.this).targetSportMinutes;
    ...
    private void getChartData() {
        ...
        for (int i = 0; i < 7; i++) {
            if (lWeekDayList.size() < (i + 1)) {
                ...
                mDays.add(new MonitorListHeadME.MonitorColumnDay(TimeU.intToWeekChinese(lWeekDayList.get(i).weekday), lWeekDayList.get(i).exercised_minutes));
                //lWeekDayList.get(i).stepcount)
                if (lMaxSportTime < lWeekDayList.get(i).exercised_minutes) {
                    lMaxSportTime = lWeekDayList.get(i).exercised_minutes;
                }
            }
        }
        mListHeadME = new MonitorListHeadME(MonitorListHeadME.TYPE_SPORT_TIME, lMaxSportTime,
                LocalApplication.getInstance().getLoginUser(MonitorSportTimeListActivity.this).targetSportMinutes, mDays);
        //lWeekDayList.get(lWeekDayList.size() - 1).target_step
        mHeadView = new MonitorListHeadView(MonitorSportTimeListActivity.this);
        mHeadView.bindHeadView(mListHeadME);
        mSportTimeLv.addHeaderView(mHeadView);
        mHasShowChart = true;
        getCurrWeekData();
    }
    ...
    public void getCurrWeekData() {
        ...
        for (int i = 0; i < lWeekDayDEList.size(); i++) {
            ...
            MonitorListAE monitorListAE = new MonitorListAE(MonitorListAE.ITEM, mSectionPosition, mListPosition, 0,
                    "", "分钟活跃时间", summaryDE.exercised_minutes, summaryDE.target_exercised_minutes, summaryDE.date, "");
            mListPosition++;
            lWeekList.add(monitorListAE);
            //计算步数和
            lWeekSportTimeSum += summaryDE.exercised_minutes;
        }
        //增加头信息，增加位置记数
        MonitorListAE monitorSectionAE = new MonitorListAE(MonitorListAE.SECTION, mSectionPosition, mListPosition, lWeekSportTimeSum,
                TimeU.formatTimeScope(mCurrLoadingDay, lWeekDayDEList.get(lWeekDayDEList.size() - 1).date), "分钟活跃时间", 0, 0, "", "");
    ...
    public void getLastWeekData() {
        ...
        for (int i = 0; i < lLastWeekDayList.size(); i++) {
            if (i % 7 == 0 && i != 0) {
                //头信息
                MonitorListAE monitorSectionAE = new MonitorListAE(MonitorListAE.SECTION, mSectionPosition,
                        mListPosition, lWeekSportTimeSum, TimeU.formatTimeScope(mCurrLoadingDay,
                        lLastWeekDayList.get(i - 1).date), "分钟活跃时间", 0, 0, "", "");
                mMonitorData.add(monitorSectionAE);
                mMonitorData.addAll(lWeekList);
                mSectionPosition++;
                mListPosition++;
            ...
            MonitorListAE monitorListAE = new MonitorListAE(MonitorListAE.ITEM, mSectionPosition,
                    mListPosition, 0, "", "分钟活跃时间", summaryDE.exercised_minutes,
                    summaryDE.target_exercised_minutes, summaryDE.date, "");
            mListPosition++;
            lWeekList.add(monitorListAE);
            //计算步数和
            lWeekSportTimeSum += summaryDE.exercised_minutes;
    ...
    private void getDayMotionSummary(final int delay, final String toDay) {
        ...
        x.task().post(new Runnable() {
            ...
            public void run() {
                ...
                RequestParams params = RequestParamsBuilder.buildGetDayMotionSummaryRP(MonitorSportTimeListActivity.this,
