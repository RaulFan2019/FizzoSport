 ...
 * 步数列表详情页面
 */
public class MonitorStepListActivity extends BaseActivity {

    /* contains */
    private static final String TAG = "MonitorStepListActivity";
    ...
    PullablePinnedSectionListView mStepLv;
    ...
    protected int getLayoutId() {
        return R.layout.activity_monitor_step_detail_list;
    }
    ...
    Handler mPostHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case MSG_POST_ERROR:
                    T.showShort(MonitorStepListActivity.this, msg.obj.toString());
                    if (!mFirstIn) {
    ...
    protected void initData() {
        mUserId = LocalApplication.getInstance().getLoginUser(MonitorStepListActivity.this).userId;
        mCurrLoadingDay = TimeU.nowDay();
    }

    @Override
    protected void initViews(Bundle savedInstanceState) {
        mAdapter = new MonitorListAdapter(MonitorStepListActivity.this, mMonitorData, MonitorListAdapter.VALUE_TYPE_INT);
        mStepLv.setAdapter(mAdapter);
...
//                doRefresh();
    ...
    protected void onResume() {
        ...
        if (mHasShowChart && !mIsFirstIn) {
            mListHeadME.targetValue = LocalApplication.getInstance().getLoginUser(MonitorStepListActivity.this).targetStepCount;
    ...
    private void getChartData() {
        ...
        for (int i = 0; i < 7; i++) {
            if (lWeekDayList.size() < (i + 1)) {
                ...
                mDays.add(new MonitorListHeadME.MonitorColumnDay(TimeU.intToWeekChinese(lWeekDayList.get(i).weekday), lWeekDayList.get(i).stepcount));
                //lWeekDayList.get(i).stepcount)
                if (lMaxStep < lWeekDayList.get(i).stepcount) {
                    lMaxStep = lWeekDayList.get(i).stepcount;
                }
            }
        }
        mListHeadME = new MonitorListHeadME(MonitorListHeadME.TYPE_STEP, lMaxStep,
                LocalApplication.getInstance().getLoginUser(MonitorStepListActivity.this).targetStepCount, mDays);
        //lWeekDayList.get(lWeekDayList.size() - 1).target_step
        mHeadView = new MonitorListHeadView(MonitorStepListActivity.this);
        mHeadView.bindHeadView(mListHeadME);
        mStepLv.addHeaderView(mHeadView);
        mHasShowChart = true;
        getCurrWeekData();
    }
    ...
    public void getCurrWeekData() {
        ...
        for (int i = 0; i < lWeekDayDEList.size(); i++) {
            ...
            MonitorListAE monitorListAE = new MonitorListAE(MonitorListAE.ITEM, mSectionPosition, mListPosition, 0,
                    "", "步", summaryDE.stepcount, summaryDE.target_step, summaryDE.date, "");
            mListPosition++;
            lWeekList.add(monitorListAE);
            //计算步数和
            lWeekStepSum += summaryDE.stepcount;
        }
        //增加头信息，增加位置记数
        MonitorListAE monitorSectionAE = new MonitorListAE(MonitorListAE.SECTION, mSectionPosition, mListPosition, lWeekStepSum,
                TimeU.formatTimeScope(mCurrLoadingDay, lWeekDayDEList.get(lWeekDayDEList.size() - 1).date), "步", 0, 0, "", "");
    ...
    public void getLastWeekData() {
        ...
        for (int i = 0; i < lLastWeekDayList.size(); i++) {
            if (i % 7 == 0 && i != 0) {
                //头信息
                MonitorListAE monitorSectionAE = new MonitorListAE(MonitorListAE.SECTION, mSectionPosition, mListPosition, lWeekStepSum,
                        TimeU.formatTimeScope(mCurrLoadingDay, lLastWeekDayList.get(i - 1).date), "步", 0, 0, "", "");
                mMonitorData.add(monitorSectionAE);
                mMonitorData.addAll(lWeekList);
                mSectionPosition++;
                mListPosition++;
    ...
    private void getDayMotionSummary(final int delay, final String toDay) {
        ...
        x.task().post(new Runnable() {
            ...
            public void run() {
                ...
                RequestParams params = RequestParamsBuilder.buildGetDayMotionSummaryRP(MonitorStepListActivity.this,
