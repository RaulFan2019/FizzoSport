...
public class RegisterActivity extends BaseActivity {
    ...
    private static final int MSG_POST_NEXT_OK = 0x02;
    ...
    private boolean mPostNextEnable;//修改密码请求
    ...
    private Callback.Cancelable mNextBtnCancelable;//取消进入下一步
    ...
    @OnClick({R.id.tv_get_v_code, R.id.btn_complete, R.id.btn_back, R.id.rl_register})
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case MSG_POST_NEXT_OK:
                    ...
                    Intent i = new Intent(RegisterActivity.this, RegisterUserInfoActivity.class);
                    i.putExtra("phone", mPhoneNum);
                    i.putExtra("password", mPsw);
                    startActivity(i);
                    break;
    ...
    protected void causeGC() {
        ...
        if (mNextBtnCancelable != null && !mNextBtnCancelable.isCancelled()) {
            mNextBtnCancelable.cancel();
        }
    ...
    private void checkPhone() {
        ...
        String lError = StringU.checkPhoneNumInputError(RegisterActivity.this, mPhoneNum);
        ...
        //进度条
        mDialog.showProgressDialog(RegisterActivity.this, "请稍后...", true);
        ...
        x.task().post(new Runnable() {
            @Override
            public void run() {
                final RequestParams requestParams = RequestParamsBuilder.buildCheckPhoneRP(RegisterActivity.this, UrlConfig.URL_CHECK_PHONE_IS_REGISTER, mPhoneNum);
    ...
    private void getVCode() {
        ...
        x.task().post(new Runnable() {
            @Override
            public void run() {
                RequestParams requestParams = RequestParamsBuilder.buildGetVCodeRP(RegisterActivity.this, UrlConfig.URL_GET_VC_CODE, mPhoneNum);
                mCheckPhoneCancelable = x.http().post(requestParams, new Callback.CommonCallback<BaseRE>() {
                    @Override
                    public void onSuccess(BaseRE result) {
                        if (mPostHandler != null) {
    ...
    private void onNextBtnClick() {
        ...
        mPsw = mPswEt.getText().toString();
        lError = StringU.checkPasswordInputError(RegisterActivity.this, mPsw);
        if (!lError.equals(AppEnum.DEFAULT_CHECK_ERROR)) {
        ...
        lError = StringU.checkVCCodeError(RegisterActivity.this, lVCode);
        if (!lError.equals(AppEnum.DEFAULT_CHECK_ERROR)) {
        ...
        x.task().post(new Runnable() {
            @Override
            public void run() {
                RequestParams params = RequestParamsBuilder.buildCheckVCCodeRP(RegisterActivity.this, UrlConfig.URL_CHECK_VC_CODE, mPhoneNum, lVCode);
                mNextBtnCancelable = x.http().post(params, new Callback.CommonCallback<BaseRE>() {
                    @Override
                    public void onSuccess(BaseRE result) {
                        if (mPostHandler != null) {
                            if (result.errorcode == BaseResponseParser.ERROR_CODE_NONE) {
                                Message msg = new Message();
                                msg.what = MSG_POST_NEXT_OK;
