 ...
 * 锻炼点数列表详情页面
 */
public class MonitorSportPointListActivity extends BaseActivity {
    ...
    PullablePinnedSectionListView mSportPointLv;
    ...
    protected int getLayoutId() {
        return R.layout.activity_monitor_sport_time_list;
    }
    ...
    Handler mPostHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case MSG_POST_ERROR:
                    T.showShort(MonitorSportPointListActivity.this, msg.obj.toString());
                    if (!mFirstIn) {
    ...
    protected void initData() {
        mUserId = LocalApplication.getInstance().getLoginUser(MonitorSportPointListActivity.this).userId;
        mCurrLoadingDay = TimeU.nowDay();
    }

    @Override
    protected void initViews(Bundle savedInstanceState) {
        mAdapter = new MonitorListAdapter(MonitorSportPointListActivity.this, mMonitorData, MonitorListAdapter.VALUE_TYPE_INT);
        mSportPointLv.setAdapter(mAdapter);
...
//                doRefresh();
    ...
    protected void onResume() {
        ...
        if (mHasShowChart && !mIsFirstIn) {
            mListHeadME.targetValue = LocalApplication.getInstance().getLoginUser(MonitorSportPointListActivity.this).targetPoint;
    ...
    private void getChartData() {
        ...
        for (int i = 0; i < 7; i++) {
            if (lWeekDayList.size() < (i + 1)) {
                ...
                mDays.add(new MonitorListHeadME.MonitorColumnDay(TimeU.intToWeekChinese(lWeekDayList.get(i).weekday), lWeekDayList.get(i).effort_point));
                if (lMaxSportPoint < lWeekDayList.get(i).effort_point) {
                    lMaxSportPoint = lWeekDayList.get(i).effort_point;
                }
            }
        }
        mListHeadME = new MonitorListHeadME(MonitorListHeadME.TYPE_SPORT_POINT, lMaxSportPoint,
                LocalApplication.getInstance().getLoginUser(MonitorSportPointListActivity.this).targetPoint, mDays);
        mHeadView = new MonitorListHeadView(MonitorSportPointListActivity.this);
        mHeadView.bindHeadView(mListHeadME);
        mSportPointLv.addHeaderView(mHeadView);
        mHasShowChart = true;
        getCurrWeekData();
    }
    ...
    public void getCurrWeekData() {
        ...
        for (int i = 0; i < lWeekDayDEList.size(); i++) {
            ...
            MonitorListAE monitorListAE = new MonitorListAE(MonitorListAE.ITEM, mSectionPosition, mListPosition, 0,
                    "", "点", summaryDE.effort_point, summaryDE.target_point, summaryDE.date, "获得");
            mListPosition++;
            lWeekList.add(monitorListAE);
            //计算步数和
            lWeekSportPointSum += summaryDE.effort_point;
        }
        //增加头信息，增加位置记数
        MonitorListAE monitorSectionAE = new MonitorListAE(MonitorListAE.SECTION, mSectionPosition, mListPosition, lWeekSportPointSum,
                TimeU.formatTimeScope(mCurrLoadingDay, lWeekDayDEList.get(lWeekDayDEList.size() - 1).date), "点", 0, 0, "", "获得");
    ...
    public void getLastWeekData() {
        ...
        for (int i = 0; i < lLastWeekDayList.size(); i++) {
            if (i % 7 == 0 && i != 0) {
                //头信息
                MonitorListAE monitorSectionAE = new MonitorListAE(MonitorListAE.SECTION, mSectionPosition, mListPosition, lWeekSportPointSum,
                        TimeU.formatTimeScope(mCurrLoadingDay, lLastWeekDayList.get(i - 1).date), "点", 0, 0, "", "获得");
                mMonitorData.add(monitorSectionAE);
                mMonitorData.addAll(lWeekList);
                mSectionPosition++;
                mListPosition++;
    ...
    private void getDayMotionSummary(final int delay, final String toDay) {
        ...
        x.task().post(new Runnable() {
            ...
            public void run() {
                ...
                RequestParams params = RequestParamsBuilder.buildGetDayMotionSummaryRP(MonitorSportPointListActivity.this,
